...
Target  : (ce_entry - candle_low) + (pe_entry - candle_low) >= TARGET_POINTS → exit at LOW
SL      : Either candle_high >= leg_entry + SL_POINTS → exit at SL price
Re-entry: Next candle open immediately after SL or Target (same fixed strike)
...
CSV_PREFIX    = "nifty_"
POLL_INTERVAL = 5        # seconds between feed checks
...
# ─────────────────────────────────────────────────────────────────────────────
# FEED READER
# Monitors today's CSV, yields only NEW completed candles.
# A candle is complete when a newer minute row appears in the CSV,
# guaranteeing no partial candle data is ever acted on.
# ─────────────────────────────────────────────────────────────────────────────
...
class FeedReader:
    ...
    def __init__(self, csv_path: str):
        ...
        self._skip_minutes = set()   # minutes present at startup — ignored
        self._seen_minutes = set()   # minutes already returned as sealed
        self._initialized  = False
        self.fixed_strike  = None
        self.expiry        = None

    def initialize(self):
        """Mark all rows currently in the CSV as past — never returned."""
    ...
    def poll(self) -> list:
        """
        Return list of newly sealed candle dicts.
        Each dict: datetime(HH:MM), spot_open, strike, ce{ohlc}, pe{ohlc}
        """
        ...
        # Only process minutes that have a NEWER minute after them (fully sealed)
        all_minutes = sorted(buckets.keys())
        sealed      = all_minutes[:-1]
    ...
    def latest_tick(self):
        """
        Latest close prices from the most recent (possibly partial) candle.
        Used only for status display — never for trade decisions.
        Returns (spot_open, ce_close, pe_close) or None.
        """
        if not os.path.exists(self.csv_path):
            return None
        try:
...
class LiveStrategy:
    ...
    def _close_trade(self, ce_exit, pe_exit, reason, hhmm):
        ...
        print(f"\n  {icon} TRADE #{t['trade_num']} CLOSED  [{hhmm}]  "
              f"{reason}  |  "
    ...
    def _process_candle(self, candle) -> bool:
        """
        Process one sealed candle. Returns True if session should end.

        Re-entry after SL/Target opens a new trade but does NOT check
        SL/Target again on the same candle — the new trade waits for the
        next candle. This matches backtest exactly: in backtest the for-loop
        moves to the next spot_row after closing, and opens the re-entry
        trade at that next candle's open.
        """
        ...
        # Open trade if flat and within window
        ...
        if exit_reason:
            ...
            # Re-entry: mark flat — new trade opens on NEXT candle's open
            # Do NOT check SL/Target again on this candle (avoids infinite loop
            # and matches backtest where the for-loop advances to next row)
            if do_reenter and in_window:
                pass   # self.active is already None after _close_trade
    ...
    def _print_status(self):
        ...
        if self.active:
            t = self.active
            ce_live   = _leg_pnl(t['ce_entry'], ce_ltp or t['ce_entry'])
            pe_live   = _leg_pnl(t['pe_entry'], pe_ltp or t['pe_entry'])
            open_pnl  = (ce_live + pe_live) * LOT_SIZE
    ...
    def run(self):
        ...
        try:
            while not self.session_done:
                ...
                # Auto-stop after 15:16 (15:15 candle will have been processed)
                if now.hour > 15 or (now.hour == 15 and now.minute >= 16):
                    print("\n🏁 Session complete.")
                    break

                # Poll for new sealed candles and process them
                ...
                # Per-minute status — prints once per minute on the clock
